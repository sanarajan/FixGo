  ...
  const fetchTransactions = async (page = 1) => {
    try {
      const res = await axiosClient.get(
        `/api/provider/transactions?page=${page}&limit=${transactionsPerPage}`
      );
      setTransactions(res.data.data); // ✅ Transaction data
      setCurrentPage(res.data.currentPage); // ✅ Page number from backend
