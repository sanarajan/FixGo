...
const menuItems: MenuItem[] = [
  ...
  { label: 'PROVIDERS', icon: <FaUsers />, path: '/admin/providers' },
  { label: 'CUSTOMERS', icon: <FaUsers />, path: '/admin/customers' },
  { label: 'SERVICES', icon: <FaCogs />, path: '/admin/services' },
...
useEffect(() => {
 ...
 const imgpath = "providerServices/";
    const API = import.meta.env.VITE_API_URL;

   const imageURL = user?.image
    ? `${API}/uploads/${imgpath}${user.image}`
    : `${API}/asset/noimage.png`; // use full path or public image path
